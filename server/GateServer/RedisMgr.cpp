...
RedisMgr::RedisMgr() {
	...
	auto host = g_config_mgr["Redis"]["host"];
	auto port = atoi(g_config_mgr["Redis"]["port"].c_str());
	auto pwd = g_config_mgr["Redis"]["passwd"];
	_conPool.reset(new RedisConPool(std::thread::hardware_concurrency(), host.c_str(), port, pwd.c_str()));
}
